...
mod_dic = {'resnet18':ResNet18, 'resnet101':ResNet101, 'resnet152':ResNet152, 'vgg':(lambda :VGG('VGG19')),
           'densenet':DenseNet121}
net = mod_dic[args.net]()
Replace(args.sim_type)(net)
print (args.net)
print (net)
net.apply(xavier)
# net = VGG('VGG19')
...
if args.resume:
    ...
    checkpoint = torch.load('./checkpoint/'+args.net+'.t7')
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/'+args.net+'.t7')
        best_acc = acc
